...
describe('BlogPage Test Cases', () => {
    ...
    it('Should render delete & logout button when JWT token cookie exist', async () => {
        ...
        // 이 부분은 data-testid={`btn-${post.id}`} 로 testid가 할당되어 있음
        expect(screen.getByTestId('btn-1')).toBeInTheDocument() 
        expect(screen.getByTestId('btn-2')).toBeInTheDocument()
    })

    it('Should render not delete & logout button when no cookie', async () => {
        const { page } = await getPage({
            route: '/',
        })
        render(page)
        expect(await screen.findByText('blog page')).toBeInTheDocument()
        // 해당 항목이 없을 때엔 queryByTestId()를 사용함
